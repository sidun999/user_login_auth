...
jQuery.fn.loadthumb = function(options) {
	...
	$(img).load(function(){
		...
		imgDem.w  = img.width;
		imgDem.h  = img.height;
		imgDem = $.imgResize({"w": $("#"+options.parentId).width()-40 ,"h": $("#"+options.parentId).height()-60},{"w":imgDem.w,"h":imgDem.h});
		var imgMargins = $.imgCenter({"w": $("#"+options.parentId).width() ,"h": $("#"+options.parentId).height()},{"w":imgDem.w,"h":imgDem.h});
		_self.css({width:imgDem.w,height:imgDem.h,left:imgMargins.l,top:imgMargins.t-5});
