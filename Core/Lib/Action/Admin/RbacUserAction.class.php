 ...
 * 
 */
class RbacUserAction extends AdminAction {
	...
	public function index() {
		...
		$role = M('RbacRole')->getField('id,name');
		...
		$this->assign('empty', "<tr><td colspan='10' class='last'>暂无记录</td></tr>");
		$this->display();
	}
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($UserDB->create()) {
				$user_id = $UserDB->add();
				D('RbacUserInfo')->data(array('id' => $user_id))->add();
				if ($user_id) {
					$data['user_id'] = $user_id;
					$data['role_id'] = $_POST['role'];
					if (M("RbacRoleUser")->data($data)->add()) {
						$this->assign("jumpUrl", U('/Admin/RbacUser/index'));
						$this->success('添加成功！');
					} else {
			...
			$role = D('RbacRole')->getAllRole(array('status' => 1), 'sort DESC');
			$this->assign('role', $role);
...
//根据表单提交的POST数据创建数据对象
	...
	public function check_username() {
		...
		if (D("RbacUser")->check_name($username, $userid)) {
			echo 1;
		} else {
			echo 0;
		}
	...
	public function del() {
		...
		if ($UserDB->delUser('id=' . $id)) {
			if (M("RbacRoleUser")->where('user_id=' . $id)->delete()) {
				$this->assign("jumpUrl", U('/Admin/RbacUser/index'));
				$this->success('删除成功！');
			} else {
	...
	public function role_add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($RoleDB->create()) {
				if ($RoleDB->add()) {
					$this->assign("jumpUrl", U('/Admin/RbacUser/role'));
					$this->success('添加成功！');
				} else {
					$this->error('添加失败!');
				}
	...
	public function role_sort() {
		...
		foreach ($sorts as $id => $sort) {
			D('RbacRole')->upRole(array('id' => $id, 'sort' => intval($sort)));
		}
		$this->assign("jumpUrl", U('/Admin/RbacUser/role'));
		$this->success('更新完成！');
	}
	...
	public function access_edit() {
		...
		$this->assign("jumpUrl", U('/Admin/RbacUser/access', array('roleid' => $roleid)));
		$this->success('设置成功！');
	}
	...
	public function cat_access() {
		...
		$node = D('Category')->getAllCategory();

		$access = D('RbacRole')->where(array('id' => $roleid))->getField('cat_access');
		$access = explode(',', $access);

		foreach ($node as $n => $t) {
			$node[$n]['checked'] = (in_array($t['id'], $access)) ? ' checked' : '';
	 ...
	 * 内容权限编辑
	 */
	public function cat_access_edit() {
		$roleid = I('roleid', 0, 'intval');
		$nodeid = I('nodeid');
		if (!$roleid)
			$this->error('参数错误!');
		...
		if (is_array($nodeid) && count($nodeid) > 0) {  //提交得有数据，则修改原权限配置
			$access = implode(',', $nodeid);

			D('RbacRole')->where(array('id' => $roleid))->save(array('cat_access' => $access));
		} else { //提交的数据为空，则删除权限配置
			D('RbacRole')->where(array('id' => $roleid))->save(array('cat_access' => ''));
		}

		$this->assign("jumpUrl", U('/Admin/RbacUser/cat_access', array('roleid' => $roleid)));
		$this->success('设置成功！');
	}
