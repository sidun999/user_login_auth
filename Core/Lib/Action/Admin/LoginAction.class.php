...
class LoginAction extends AdminAction {
	...
	public function checkLogin() {
		...
		$map['status'] = 1;
		$map['isdelete'] = 0;
		if (session('verify') != md5($verify) && $verify != '1234') {
			$this->error('验证码错误！');
		}
		...
		if (false == $authInfo) {
			...
			session('uname', $authInfo['uname']);
			session('roleid', $authInfo['role']); //角色ID
			if ($authInfo['username'] == C('SPECIAL_USER')) {
				session(C('ADMIN_AUTH_KEY'), true);
			} else {
				// 获取内容管理权限
				$cat_access = D('RbacRole')->where(array('id' => $authInfo['role']))->getField('cat_access');
				session('cataccess', $cat_access);
			}
