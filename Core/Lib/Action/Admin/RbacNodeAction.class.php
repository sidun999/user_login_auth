 ...
 * 
 */
class RbacNodeAction extends AdminAction {
	...
	public function index() {
		...
		foreach ($Node as $k => $r) {
			...
			$r['submenu'] = $r['level'] == 3 ? '<font color="#cccccc">添加子菜单</font>' : "<a href='" . U('/Admin/RbacNode/add/pid/' . $r['id']) . "'>添加子菜单</a>";
			$r['edit'] = $r['level'] == 1 ? '<font color="#cccccc">修改</font>' : "<a href='" . U('/Admin/RbacNode/edit/id/' . $r['id'] . '/pid/' . $r['pid']) . "'>修改</a>";
			$r['del'] = $r['level'] == 1 ? '<font color="#cccccc">删除</font>' : "<a onClick='return confirmurl(\"" . U('/Admin/RbacNode/del/id/' . $r['id']) . "\",\"确定删除该菜单吗?\")' href='javascript:void(0)'>删除</a>";
			switch ($r['display']) {
				case 0:
					$r['display'] = '不显示';
					break;
		...
		$str = "<tr>
				    ...
				    <td>\$id</td> 
				    <td title='\$$name'>\$spacer \$title (\$data)</td>
				    <td>\$level</td> 
				    <td>\$status</td> 
				    <td>\$display</td> 
					<td class='last'>
	...
	public function add() {
		if (isset($_POST['dosubmit'])) {
			...
			if ($NodeDB->create()) {
				if ($NodeDB->add()) {
					$this->assign("jumpUrl", U('/Admin/RbacNode/index'));
					$this->success('添加成功！');
				} else {
					$this->error('添加失败!');
				}
	...
	public function sort() {
		...
		foreach ($sorts as $id => $sort) {
			D('RbacNode')->upNode(array('id' => $id, 'sort' => intval($sort)));
		}
		$this->assign("jumpUrl", U('/Admin/RbacNode/index'));
		$this->success('更新完成！');
	}
